<?php
	...
	class Database
	{
		 ...
		 */
		public function __construct($db_driver = FALSE, $db_name = FALSE, $db_user = FALSE, $db_password = FALSE)
		{	
			try {
		 ...
		 */
		private static function GetFetch($tablename, $select, $where)
		{
			...
			if($where_total >0) {
				$where_keys = array_keys($where_arg);
				$sql .= ' WHERE ';
				for($i=0; $i < $where_total; $i++) {
					$sql.= $where_keys[$i] . ' = ? ';
					if(($i + 1) <$where_total) $sql .= ' AND ';
				}
			}
			
			$sth = $dbh->prepare($sql);
			if($where_total > 0) {
				$where_values = array_values($where_arg);
				for($i = 0;$i<$where_total;$i++) {
					$sth->bindValue(($i + 1), self::Filter($where_values[$i]));
				}
			}
		 ...
		 * @author Kevin van Steijn, Maarten Oosting
		 */
		public static function GetSingle($tablename, $select = '*', $where = array())
		{
			try {
				$dbh = self::$_dbh;
				if (!$dbh) throw new PDOException('No database connection');
				
				$sth = self::GetFetch($tablename, $select, $where);
		 ...
		 * @author Kevin van Steijn, Maarten Oosting
		 */
		public static function GetArray($tablename, $select = '*', $where = array())
		{
			try{
				$dbh = self::$_dbh;
				if (!$dbh) throw new PDOException('No database connection');
		...
		public static function Update($tablename, $update_arg, $where_arg = array())
		{
			try {
				...
				$update_values = array_values($update_arg);
				for($i = 0; $i < $update_total; $i++) {
					$sth->bindValue(($i + 1), strip_tags($update_values[$i]));
				}
				
				if ($where_total > 0) {
					$where_values = array_values($where_arg);
					for($i = 0; $i < $where_total; $i++) {
						$sth->bindValue(($update_total + $i + 1), strip_tags($where_values[$i]));
					}
				}
		...
		public static function Insert($tablename, $arguments)
		{
			try {
				...
				for ($i = 0; $i < $total; $i++) {
					$sth->bindValue(($i + 1), self::Filter($values[$i]));
				}
